...
export default function DeleteServer() {
  ...
  const onClickhandle = async () => {
    setisloading(true)
    try {
      const res = await axios.delete(`/api/admin/server/${server?.server?._id}/delete`, {
        data: {
      ...
      <div className="fixed top-28 left-[580px]  transform -translate-x-1/2 z-50 ">
        <div className=" border border-gray-800 rounded-lg shadow-xl w-full max-w-md p-6 text-gray-100 max-h-[90vh] overflow-y-auto relative">
          ...
          <div className="text-center">
            <h2 className="text-xl font-bold text-gray-100">Delete Server</h2>
            <p className="text-sm mt-2 text-gray-400">
            Are you sure you want to Delete <span className="text-indigo-400">{server?.server?.name}</span>? Server will be deleted permanently and you will not be able to recover it.
          </p>
